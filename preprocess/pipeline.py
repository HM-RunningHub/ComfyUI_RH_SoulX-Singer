...
from .utils import convert_metadata, merge_short_segments
...
class PreprocessPipeline:
    def __init__(
        ...
        model_path: str = "/workspace/ComfyUI/models/Soul-AILab/SoulX-Singer-Preprocess",
    ):
        self.device = device
        ...
        if vocal_sep:
            self.vocal_separator = VocalSeparator(
                sep_model_path=f"{self.model_path}/mel-band-roformer-karaoke/mel_band_roformer_karaoke_becruily.ckpt",
                sep_config_path=f"{self.model_path}/mel-band-roformer-karaoke/config_karaoke_becruily.yaml",
                der_model_path=f"{self.model_path}/dereverb_mel_band_roformer/dereverb_mel_band_roformer_anvuew_sdr_19.1729.ckpt",
                der_config_path=f"{self.model_path}/dereverb_mel_band_roformer/dereverb_mel_band_roformer_anvuew.yaml",
                device=device
            )
        else:
            self.vocal_separator = None
        self.f0_extractor = F0Extractor(
            model_path=f"{self.model_path}/rmvpe/rmvpe.pt",
            device=device,
        )
        ...
        self.lyric_transcriber = LyricTranscriber(
            zh_model_path=f"{self.model_path}/speech_seaco_paraformer_large_asr_nat-zh-cn-16k-common-vocab8404-pytorch",
            en_model_path=f"{self.model_path}/parakeet-tdt-0.6b-v2/parakeet-tdt-0.6b-v2.nemo",
            device=device
        )
        self.note_transcriber = NoteTranscriber(
            rosvot_model_path=f"{self.model_path}/rosvot/rosvot/model.pt",
            rwbd_model_path=f"{self.model_path}/rosvot/rwbd/model.pt",
            device=device
        )
    ...
    def run(
        ...
        # return audio_path.replace(".wav", ".json").replace(".mp3", ".json").replace(".flac", ".json")
        metadata = json.dumps(final_metadata, ensure_ascii=False, indent=2)
        return metadata
