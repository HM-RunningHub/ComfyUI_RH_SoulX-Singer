...
class LlamaNARDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        # Self Attention
        attn_signature = inspect.signature(self.self_attn.forward).parameters
        attn_kwargs = {
            "hidden_states": hidden_states,
            "attention_mask": attention_mask,
        }

        # HF transformers >= 4.46 expects precomputed rotary position embeddings.
        if "position_embeddings" in attn_signature:
            if position_embeddings is None:
                raise ValueError(
                    "position_embeddings is required for current transformers attention API."
                )
            attn_kwargs["position_embeddings"] = position_embeddings
            if "past_key_values" in attn_signature:
                attn_kwargs["past_key_values"] = past_key_value
            if "cache_position" in attn_signature:
...
class DiffLlama(LlamaModel):
    def __init__(
        ...
        def _build_layer_config() -> LlamaConfig:
            cfg = LlamaConfig(
            ...
            # transformers>=4.52 expects this internal field to select the attention backend.
